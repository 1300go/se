...
본 시스템은 구조 모델링의 일관성을 확보하고 시스템 아키텍처 레이어를 명확히 분리하기 위해, 순수 비즈니스 개념을 담은 **엔티티 클래스(Entity Model)**와 데이터 영속성을 처리하는 **데이터 접근 객체(DAO / Repository)**를 다음과 같이 엄격히 분리하여 설계한다.

#### 3.1.1 도메인 엔티티(Entity) 모델 정의
* **Account (추상 클래스):** 시스템 접근 주체의 공통 속성(userId, password)을 정의함.
* **User (일반 사용자):** 독립 디렉터리 소유 및 파일 제어 권한을 가진 순수 도메인 객체.
* **Admin (관리자):** 사용자 계정 생성 및 권한 변경 규칙을 내포한 도메인 객체.
* **File (파일):** 파일 고유 메타데이터(파일명, 크기, 유형) 및 비즈니스 로직을 포함함.
* **Folder (폴더):** 계층형 디렉터리 구조 구성을 위한 도메인 객체.
* **Version (버전):** 파일의 변경 이력 시점 스냅샷 정보를 보존함.

#### 3.1.2 데이터 접근 객체(DAO / Repository) 레이어 정의
기존 초안의 '사용자DB', '버전DB' 등 데이터베이스 자체를 의미하던 혼용 표기를 아키텍처 표준에 맞추어 데이터 접근 인터페이스 객체로 전면 정정한다.
* **UserRepository:** `User` 및 `Admin` 엔티티의 영속성 관리를 전담하는 DAO 객체.
* **FileRepository:** 스토리지 내 물리적 파일 경로 제어 및 `File`, `Folder` 엔티티 데이터를 처리하는 DAO 객체.
* **VersionRepository:** 파일 변경 시 생성되는 `Version` 엔티티의 이력 조회를 전담하는 DAO 객체.
* **AuditLogRepository:** 파일 접근 및 수정 행위를 감사 데이터베이스에 실시간으로 영속화하는 DAO 객체.

#### 3.1.3 레이어 간 관계성 및 가시성 명세
1. **상속 관계 (Generalization):** `User`와 `Admin`은 부모 엔티티 클래스인 `Account`를 상속받는다.
2. **포함 관계 (Composition):** `Folder` 엔티티는 하위 `Folder` 및 `File` 엔티티를 계층적으로 포함한다.
3. **의존 관계 (Dependency):** 비즈니스 로직을 제어하는 Controller 클래스들이 각 엔티티 모델을 생성 및 가공한 후, 대응하는 Repository(DAO) 객체의 메서드를 호출하여 데이터 영속화를 수행한다.
4. **가시성(Visibility):** 모든 엔티티의 멤버 변수는 무결성 보호를 위해 `private(-)`으로 은닉하며, Repository 객체의 데이터 입출력 인터페이스 메서드는 `public(+)`으로 노출한다.
```
...
기능 모델링에서 도출된 유스케이스 흐름에 따라, 시간의 흐름을 중심으로 한 객체 간의 함수 호출 시나리오를 다음과 같이 명세한다. 각 흐름은 일관성을 위해 **Actor ➔ Boundary(UI) ➔ Controller ➔ Entity/DB** 순서로 진행된다.
